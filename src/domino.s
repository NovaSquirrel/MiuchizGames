...
PuzzleZeroStart:
  PuzzleState:  .res PLAYERS    ; State each playfield is in
  ; Experimental swap stuff
  PuzzleSwapX: .res PLAYERS
  PuzzleSwapY: .res PLAYERS
  PuzzleSwapMode: .res PLAYERS  ; if players are in swap mode or not

  ; Send garbage
  PuzzleMatchesMade: .res PLAYERS ; matches made between dropping each piece
  PuzzleMatchColor:  .res PLAYERS*4 ; first player's four colors, then second player's

  ; Receive garbage
  PuzzleGarbageCount: .res PLAYERS
  PuzzleGarbageColor: .res PLAYERS*4
  LockoutSoftDrop:    .res PLAYERS ; don't allow soft drop until you press down again
PuzzleZeroEnd:


  PuzzleX: .res PLAYERS
  PuzzleY: .res PLAYERS
  PuzzleFallTimer: .res PLAYERS ; Time until the piece falls down one row
  PuzzleDir: .res PLAYERS       ; Direction
  PuzzleColor1: .res PLAYERS
  PuzzleColor2: .res PLAYERS
  PuzzleNextColor1: .res PLAYERS
  PuzzleNextColor2: .res PLAYERS

  ; Low, medium or high
  PuzzleSpeed:  .res PLAYERS         ; ranges 0-2. pills
  PuzzleGravitySpeed: .res PLAYERS   ; ranges 0-2, gravity

  VirusLevel:   .res PLAYERS    ; Number of viruses to clear, in this version
  PuzzleRedraw: .res PLAYERS    ; Redraw entire grid
  ...
  PuzzleMap: .res PUZZLE_WIDTH*PUZZLE_HEIGHT*2 ; 256 bytes?
...
.proc PuzzleInSwapMode
  ...
  @EmptyOK:
    ...
    inc v0 ; Try the tile to the right
    jsr PuzzleGridRead
...
.proc PuzzleGravity
  Column = v0
  Row = v1
  DidFix = v2 ; flag to say a tile was fixed
  VirusCount = v3 ; used to detect winning
